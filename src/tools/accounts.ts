...
export function registerAccountTools(
  ...
  server.tool('get_ad_accounts', 'List accessible ad accounts', GetAdAccountsSchema,
    withErrorHandling('get_ad_accounts', logger, async (params) => {
      ...
      const text = JSON.stringify({
        data: res.data,
        pagination: { next_cursor: res.pagination.next_cursor, has_next: res.pagination.has_next },
      });
  ...
  server.tool('get_instagram_accounts', 'Get Instagram accounts linked to ad account', GetInstagramAccountsSchema,
    withErrorHandling('get_instagram_accounts', logger, async (params) => {
